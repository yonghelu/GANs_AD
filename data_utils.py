...
def kdd99(seq_length, seq_step, num_signals):
    ...
    samples = train[:, 0:n - 1]
    labels = train[:, n - 1]  # the last column is label
    ############################
    ############################
    # -- apply PCA dimension reduction for multi-variate GAN-AD -- #
    from sklearn.decomposition import PCA
...
def get_data(data, seq_length, seq_step, num_signals, sub_id, eval_single, eval_an, data_options=None):
    """
    ...
    print('Generated/loaded', len(samples), 'samples from data set: ', data)
    return samples, labels, index
